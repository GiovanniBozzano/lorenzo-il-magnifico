...
class PacketListener extends Thread
{
	...
	static {
		...
		PacketListener.PACKET_HANDLERS.put(PacketType.GAME_STARTED, packet -> Client.getInstance().getConnectionHandler().handleGameStarted(((PacketGameStarted) packet).getExcommunicationTiles(), ((PacketGameStarted) packet).getPlayersData(), ((PacketGameStarted) packet).getOwnPlayerIndex()));
		PacketListener.PACKET_HANDLERS.put(PacketType.GAME_PERSONAL_BONUS_TILE_CHOICE_REQUEST, packet -> Client.getInstance().getConnectionHandler().handleGamePersonalBonusTileChoiceRequest(((PacketGamePersonalBonusTileChoiceRequest) packet).getPersonalBonusTilesInformations()));
