...
public class CLIHandlerHarvest implements ICLIHandler
{
	...
	private void showFamilyMember()
	{
		...
		for (AvailableAction availableAction : GameStatus.getInstance().getCurrentAvailableActions().get(ActionType.HARVEST)) {
			if (!familyMemberTypes.containsValue(((AvailableActionFamilyMember) availableAction).getFamilyMemberType())) {
				index++;
				familyMemberTypes.put(index, ((AvailableActionFamilyMember) availableAction).getFamilyMemberType());
